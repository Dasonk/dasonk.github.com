...
```{r plotdat, echo=FALSE}
plot(x, y, main = "Scatterplot of data")
```
...
plot(x, y, main = "Scatterplot with estimated regression curve", xlim = c(0,10))
...
legend("topleft", legend = c("True max", "Estimated max", "Estimated curve"), col = c("black", "blue", "red"), lty = c(1,2,1))
```

So our estimate of the value where the maximum occurs is $$\hat{x}_\text{max} =$$ `r estmax`.  This is pretty close but it would still be nice to have some sort of interval to go along with our estimate.  This is where [the delta method](http://en.wikipedia.org/wiki/Delta_method) can help us out.  The delta method can be thought of as a way to get an estimated standard error for a transformation of estimated parameter values.  In our case we're interested in applying the function
...
`r # Add section on theory behind delta method and how to do it 'by hand'`
...
`r # Add more about using the deltamethod function`

```{r deltamethod, cache=FALSE}
library(msm)
standerr <- deltamethod(~ -x2 / (2 * x3), coef(o), vcov(o))
standerr
# Make a confidence interval
(ci <- estmax + c(-1, 1)*qnorm(.975)*standerr)
```
...
```{r hyptest}
# If we want to do a hypothesis test of
# Ho: xmax = 5
# Ha: xmax != 5
z <- (estmax - 5)/standerr
# Calculate p-value
...
Our p-value of `r round(pval, 3)` doesn't allow us to reject the null hypothesis in this situation.
